...
def logLikelihood(theta, Wavelength, WavelengthLower, WavelengthUpper, Spectrum, SpectrumErr):
    ...
    if "HJ" in CurrentSaveName:
        #Add prior to HJ
        T0_Prior = (T0-500.)**2/32.**2
        ALR_Prior = (-2.-LogALR)**2/0.34**2
        TInf_Prior = (TInf-600.)**2/11.**2
    elif "SE" in CurrentSaveName:
        #Add prior to super-Earth
        T0_Prior = (T0-300.)**2/10.**2
        ALR_Prior = (-2.-LogALR)**2/0.12**2
        TInf_Prior = (TInf-400.)**2/3.57**2

    Residual+=(T0_Prior+ALR_Prior+TInf_Prior)
    ChiSqr = -0.5*Residual

    global Iteration, nWalkers, CurrentSaveInterval
    ...
    if Residual<LeastResidual:
       ...
       with open("MCMCParams/BestParam"+CurrentSaveName+"Prior.txt", 'w+') as f:
       ...
       if 1==1:
          ...
          plt.tight_layout()
          plt.savefig("Figures/CurrentBestModel_Prior_%s.png" %CurrentSaveName)
          plt.close('all')
...
def RunMCMC( PlanetParamsDict, StellarParamsDict, CSLocation=None, AssignedzStep=0.25, SubFolderName="CS_1", SaveName="Default", NSteps=5000, NCORES=4, NewStart=False, SaveInterval=25):
    ...
    plt.savefig("Figures/BestModel_"+SaveName+"_Prior.png")
    plt.close('all')
