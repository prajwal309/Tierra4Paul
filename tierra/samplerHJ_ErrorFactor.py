...
def logLikelihood(theta, Wavelength, WavelengthLower, WavelengthUpper, Spectrum, SpectrumErr):
    ...
    logf, T0, LogALR, TInf, N0_N2Log, N0_COLog, N0_H2OLog, \
    ...
    SpecFeature = BinnedModel - (CurrentSystem.Rp/CurrentSystem.Rs)**2.0*1e6

    fValue = 10**logf
    STD = SpectrumErr*SpectrumErr+(fValue*np.abs(SpecFeature))**2

    Residual = np.sum((Spectrum-BinnedModel)**2/STD)
    ChiSqr = -0.5*Residual+np.log(np.sum(STD))
    ...
    if Residual<LeastResidual:
       ...
       with open("MCMCParams/f_BestParam"+CurrentSaveName+".txt", 'w+') as f:
       ...
       if 1==1:
          ...
          plt.ylabel("(Rp/Rs)^2")
          plt.title("ChiSqr::"+ str(ChiSqr))
          plt.subplot(312)
          NewSpectrumErr = (1.+fValue)*SpectrumErr
          plt.errorbar(Wavelength, Spectrum, yerr=NewSpectrumErr, linestyle='None')
          plt.plot(Wavelength, BinnedModel, "r-")
          plt.xlim([min(Wavelength),max(Wavelength)])
          plt.ylabel("(Rp/Rs)^2 (New Error)")
          plt.subplot(313)
...
def RunMCMC( PlanetParamsDict, StellarParamsDict, CSLocation=None, AssignedzStep=0.25, SubFolderName="CS_1", SaveName="Default", NSteps=1500, NCORES=4):
    ...
    if "HJ" in CurrentSaveName:
        f_Init = np.random.normal(-10,0.25,nWalkers)
        T0Init = np.random.normal(534., 20., nWalkers)                      #Temperature at Rp
        ALRInit = np.random.normal(-3.3481, 0.25, nWalkers)         #Adiabatic Lapse Rate in [K.km^{-1}]
        TInfInit = np.random.normal(750, 5., nWalkers)                   #Temperature in space in [K]:
        N0_N2LogInit = np.random.normal(5.1, 0.5, nWalkers)                              #Mixing ratio for nitrogen
        N0_COLogInit = np.random.normal(19.47, 0.5, nWalkers)                              #Mixing ratio for carbonmonoxide
        N0_H2OLogInit = np.random.normal(18.95, 0.5, nWalkers)                            #Mixing ratio for water
        N0_CO2LogInit = np.random.normal(18.51, 0.5, nWalkers)                            #Mixing ratio for carbondioxide
        N0_CH4LogInit = np.random.normal(18.505, 0.5, nWalkers)                            #Mixing ratio for methane
        N0_O3LogInit = np.random.normal(17.608, 0.5, nWalkers)                              #Mixing ratio for oxygen
        N0_H2LogInit = np.random.normal(23.501, 0.5, nWalkers)                              #Mixing ratio for oxygen
    elif "SE" in CurrentSaveName:
        f_Init = np.random.normal(-4,0.25,nWalkers)
        T0Init = np.random.normal(159.606, 20., nWalkers)                      #Temperature at Rp
        ALRInit = np.random.normal(-1.706, 0.25, nWalkers)         #Adiabatic Lapse Rate in [K.km^{-1}]
        TInfInit = np.random.normal(384.96, 20., nWalkers)                   #Temperature in space in [K]:
        N0_N2LogInit = np.random.normal(23.505, 0.05, nWalkers)                              #Mixing ratio for nitrogen
        N0_COLogInit = np.random.normal(21.474, 0.05, nWalkers)                              #Mixing ratio for carbonmonoxide
        N0_H2OLogInit = np.random.normal(22.638, 0.05, nWalkers)                            #Mixing ratio for water
        N0_CO2LogInit = np.random.normal(20.373, 0.05, nWalkers)                            #Mixing ratio for carbondioxide
        N0_CH4LogInit = np.random.normal(22.021, 0.05, nWalkers)                            #Mixing ratio for methane
        N0_O3LogInit = np.random.normal(19.498, 0.05, nWalkers)                              #Mixing ratio for oxygen
        N0_H2LogInit = np.random.normal(23.791, 0.05, nWalkers)                              #Mixing ratio for oxygen

    global ParameterNames
    ParameterNames = ["f", "T0", "Log_ALR", "TInf", "N0_N2Log", "N0_COLog", \
    "N0_H2OLog", "N0_CO2Log",  "N0_CH4Log", "N0_O3Log", "N0_H2Log"]

    StartingGuess = np.column_stack((f_Init, T0Init, ALRInit, TInfInit, \
    ...
    print("Saving the samplers")
    SaveMCMCName = "MCMC_Data/f_"+SaveName+".npy"
    ...
    Bestf, BestT0, BestALR, BestTInf, BestN0_N2Log, BestN0_COLog, BestN0_H2OLog, \
    ...
    SaveFigName = "Figures/Corner_f_"+SaveName+".png"
